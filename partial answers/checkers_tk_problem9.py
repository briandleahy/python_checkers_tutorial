...
import math
# and I create a draw_king function:
def _calc_ngram_pts(n, m):
    """
    Parameters
    ----------
    n, m : Int
        Values for an (n, m) gram, i.e. a star with `n` points, each
        connected by a line which skips `m` points
    """
    phis = [(i*2*math.pi) / n for i in xrange(n)]
    inds = [(i*m) % n for i in xrange(n)]
    pts = [[math.cos(phis[i]), math.sin(phis[i])] for i in inds]
    return pts

def _draw_king(canvas, x0, y0, x1, y1, ngram=(8, 3), radexpand=1.07,
        **kwargs):
    """
    Parameters
    ----------
    canvas : Tk.Canvas
        The canvas to draw the king on
    x0, y0, x1, y1 : int
        The position in pixels of the bounding box of the drawn king.
    ngram : Tuple of 2 ints
        Draws a king as an (n, m) gram. Default is (8, 3) -- i.e. an octagram
    radexpand : Float
        Expands the radius of the king by an amount ``radexpand``, keeping
        the king centered on ``(x0+x1)/2, (y0+y1)/2``. Default is 1.07
        which produces a nice flush king.

    Returns
    -------
    id : Int
        The id of the king object on the canvas.
    """
    # We draw an (n, m) gram on a circle:
    radius = (x1 - x0) * 0.5 * radexpand
    pts = _calc_ngram_pts(*ngram)
    pts_shift = []
    xm, ym = (x0 + x1)/2, (y0 + y1)/2
    for p in pts:
        pts_shift.extend([int(p[0]*radius + xm), int(p[1]*radius + ym)])
    id = canvas.create_polygon(*pts_shift, **kwargs)
    return id
...
class CheckersGUI(tk.Frame):
    ...
    def draw_piece(self, position, color, king=False):
        ...
        if not king:
            ...
            id = _draw_king(self.canvas, pt[0] + shift, pt[1] - shift -
                    self.piecesize, pt[0] + shift + self.piecesize, pt[1] -
                    shift, fill=col, outline=PIECE_LINE)
    ...
    def move_piece(self, old_position, new_position):
        ...
        id, color, king = self.pos_dict[old_position]
        self.delete_piece(old_position)
        self.draw_piece(new_position, color, king=king)
